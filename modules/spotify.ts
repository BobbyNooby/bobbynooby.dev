...
export const errorSong: SpotifySongData = {
  ...
  albumImageUrl:
    "https://i.scdn.co/image/ab67616d0000b2730e4888261e7b8c069eff0eb7",
...
export class SpotifyClient {
  ...
  startPolling() {
    ...
    this.interval = setInterval(async () => {
      try {
        ...
        // Log only, never refresh here: if the token endpoint itself is
        // failing, refreshing every 5s turns one error into a storm.
        // Token expiry is handled by the 401 branch inside checkCurrentSong.
  ...
  async checkCurrentSong() {
    ...
    if (res.status === 401) {
      // Token expired. Refresh and let the next poll (5s) use the new one.
      await this.refreshToken();
      return;
    }

    if (res.status === 204) {
      // Nothing playing; the token is fine, so just show the last played song.
      await this.sendLastPlayedFromDB();
      return;
    }

    if (res.status > 400) {
      // Rate limit or transient error: fall back to the DB without
      // touching the token endpoint.
      this.consoleBob(`Currently-playing returned ${res.status}, using DB`);
      await this.sendLastPlayedFromDB();
      return;
    }

    const song = await res.json();
    // Podcast episodes have no artists array, and a missing item means an
    // unusable payload — either way fall back to the last played song.
    if (!song?.item || song.item.type === "episode") {
      await this.sendLastPlayedFromDB();
      return;
    }
